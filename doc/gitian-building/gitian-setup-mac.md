...
The build script we'll be using is contained in the BLOBFISH github repository ([contrib/gitian-build.py](https://github.com/peppapow-project/peppapow/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the BLOBFISH repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/BLOBFISH-Project/BLOBFISH/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, peppapow-detached-sigs, and peppapow GitHub repos.
