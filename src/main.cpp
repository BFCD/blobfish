...
#if defined(NDEBUG)
#error "BLOBFISH cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
   int64_t nSubsidy = 0;
    ...
    if (nHeight == 1) {
        nSubsidy = 500000000 * COIN;    // premine 500.000.000k
	} else if (nHeight <= 100) {
        nSubsidy = 0 * COIN;	
    } else if (nHeight <= 7168) {
        nSubsidy = 10000 * COIN;
    } else {
        nSubsidy = 5000 * COIN;
    }
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetHash(fInitialSyncDone), block.nBits))
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        ...
        if ((block.nTime == (uint32_t) consensus.nBlobfishBadBlockTime) &&
            ...
            // accept BLOBFISH block minted with incorrect proof of work threshold
            return true;
        }
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    #if 0
    (pindexPrev) { // pindexPrev is only null on the first block which is a version 1 block.
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // BLOBFISH: We use certain sporks during IBD, so check to see if they are
