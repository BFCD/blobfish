...
// Copyright (c) 2015-2019 The BLOBFISH developers
...
bool AppInit(int argc, char* argv[])
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("Blobfish Core Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n";

        if (mapArgs.count("-version")) {
            strUsage += LicenseInfo();
        } else {
            strUsage += "\n" + _("Usage:") + "\n" +
                        "  blobfishd [options]                     " + _("Start Blobfish Core Daemon") + "\n";
    ...
    try {
        ...
        if (fCommandLine) {
            fprintf(stderr, "Error: There is no RPC client functionality in blobfishd anymore. Use the blobfish-cli utility instead.\n");
            exit(1);
        }
#ifndef WIN32
        fDaemon = GetBoolArg("-daemon", false);
        if (fDaemon) {
            fprintf(stdout, "BLOBFISH server starting\n");
...
int main(int argc, char* argv[])
{
    ...
    // Connect blobfishd signal handlers
    noui_connect();
