...
DashboardWidget::DashboardWidget(BLOBFISHGUI* parent) :
    PWidget(parent),
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &BLOBFISHGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &BLOBFISHGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &BLOBFISHGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("BLOBFISH")
    updateDisplayUnit();
}
