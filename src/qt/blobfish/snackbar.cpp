...
#include "qt/blobfish/forms/ui_snackbar.h"
#include "qt/blobfish/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(BLOBFISHGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &BLOBFISHGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
